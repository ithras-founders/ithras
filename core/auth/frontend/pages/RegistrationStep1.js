...
const DRAFT_KEY = 'ithras_registration_draft';

const RegistrationStep1 = ({ onSuccess, onShowLogin }) => {
  ...
  const saveDraft = () => {
    try {
      localStorage.setItem(DRAFT_KEY, JSON.stringify({
        fullName, email, username, dateOfBirth,
      }));
    } catch (_) {}
  };
    ...
    <div className="min-h-screen flex bg-[var(--app-bg)]">
      <div className=${AUTH_HERO_COLUMN_CLASS} style=${AUTH_HERO_PANEL_STYLE}>
        <div className="max-w-md text-center relative z-[1]">
          ...
          <h1 className="ith-heading-1 text-2xl md:text-3xl lg:text-4xl !mb-0 font-bold text-white tracking-tight leading-[1.15]" style=${{ fontFamily: 'var(--font-display, inherit)' }}>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center p-8 md:p-16 bg-[var(--app-surface)] overflow-y-auto custom-scrollbar border-l border-[var(--app-border-soft)]">
        ...
        <div className="max-w-sm w-full mx-auto">
          <div className="w-full h-1 rounded-full bg-[var(--app-surface-subtle)] mb-4 overflow-hidden">
            <div className="h-full rounded-full bg-[var(--app-accent)]" style=${{ width: '33.33%' }} />
          </div>
          <p className="text-xs font-medium text-[var(--app-text-muted)] mb-1">Step 1 of 3</p>
          <h2 className="text-2xl font-semibold text-[var(--app-text-primary)] mb-1 tracking-tight">Start building your professional identity</h2>
          ...
          <form onSubmit=${handleSubmit} className="space-y-4">
            ...
            ${onShowLogin ? html`
              <p className="text-center text-[15px] text-[var(--app-text-secondary)]">
