/**
 * Public community requests admin - approve, reject, request changes.
 */
...
import {
  ...
  requestChangesCommunityRequest,
} from './services/communityAdminApi.js';
import EmptyState from './components/EmptyState.js';

const html = htm.bind(React.createElement);

const CommunityRequestList = ({ onNavigateToCommunity }) => {
  ...
  const load = () => {
    setLoading(true);
    setError('');
    listCommunityRequests({ status: filter || undefined })
      .then(({ items, total: t }) => {
        setRequests(items);
        setTotal(t);
      })
  ...
  const handleApprove = async (req) => {
    try {
      const res = await approveCommunityRequest(req.id);
      load();
      if (res?.id && onNavigateToCommunity) onNavigateToCommunity(res.id);
    } catch (e) {
  ...
  const handleReject = async (req) => {
    if (!confirm(`Reject request "${req.name}"?`)) return;
    try {
      await rejectCommunityRequest(req.id);
      load();
    } catch (e) {
  ...
  const handleRequestChanges = (req) => { setChangesModal(req); setChangesMessage(''); };
  const submitChanges = async () => {
    if (!changesModal) return;
    try {
      await requestChangesCommunityRequest(changesModal.id, changesMessage);
  ...
  return html`
    <div>
      <h1 className="text-2xl font-bold mb-6" style=${{ color: 'var(--app-text-primary)' }}>Community Requests</h1>
          ...
          <table className="w-full text-sm">
            <thead style=${{ background: 'var(--app-surface-subtle)' }}>
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold" style=${{ color: 'var(--app-text-primary)' }}>Purpose</th>
            ...
            <tbody>
              ${requests.map((req) => html`
                <tr key=${req.id} className="border-t" style=${{ borderColor: 'var(--app-border-soft)' }}>
                  ...
                  <td className="px-4 py-3 max-w-[200px] truncate" style=${{ color: 'var(--app-text-secondary)' }}>${req.description || '—'}</td>
                  <td className="px-4 py-3 max-w-[120px] truncate" style=${{ color: 'var(--app-text-secondary)' }}>${req.purpose || req.targetAudience || '—'}</td>
                  ...
                  <td className="px-4 py-3 text-xs" style=${{ color: 'var(--app-text-muted)' }}>${req.createdAt ? new Date(req.createdAt).toLocaleDateString() : '—'}</td>
                  <td className="px-4 py-3 text-right">
                    ${req.status === 'pending' ? html`
                      <div className="flex gap-1 justify-end">
                        <button onClick=${() => handleApprove(req)} className="px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800 hover:bg-green-200">Approve</button>
                        <button onClick=${() => handleRequestChanges(req)} className="px-2 py-1 rounded text-xs font-medium hover:bg-[var(--app-surface-hover)]" style=${{ color: 'var(--app-text-secondary)' }}>Request changes</button>
                        <button onClick=${() => handleReject(req)} className="px-2 py-1 rounded text-xs font-medium hover:bg-red-50" style=${{ color: 'var(--app-danger)' }}>Reject</button>
                      </div>
      ...
      ${changesModal ? html`
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style=${{ background: 'rgba(0,0,0,0.4)' }} onClick=${() => setChangesModal(null)}>
          <div className="rounded-xl border bg-white shadow-xl max-w-md w-full p-6" style=${{ borderColor: 'var(--app-border-soft)' }} onClick=${(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4" style=${{ color: 'var(--app-text-primary)' }}>Request changes: ${changesModal.name}</h3>
            <textarea
            ...
            <div className="flex gap-2 justify-end">
              <button onClick=${() => setChangesModal(null)} className="px-4 py-2 rounded-lg text-sm font-medium border" style=${{ borderColor: 'var(--app-border-soft)', color: 'var(--app-text-secondary)' }}>Cancel</button>
              <button onClick=${submitChanges} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style=${{ background: 'var(--app-accent)' }}>Submit</button>
            </div>
          </div>
        </div>
      ` : null}
