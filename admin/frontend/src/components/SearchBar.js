 ...
 * Rounded corners, muted border/background, good focus state.
 */
...
/**
 * @param {{ placeholder?: string, className?: string, value?: string, onChange?: (v: string) => void }} props
 */
const SearchBar = ({ placeholder = 'Search...', className = '', value: controlledValue, onChange }) => {
  ...
  return html`
    <div className=${`relative w-full min-w-0 ${className}`}>
      <span
        className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--app-text-muted)] pointer-events-none"
        aria-hidden
      >
