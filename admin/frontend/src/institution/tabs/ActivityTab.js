/**
 * Activity tab: Table of actions (Action, User, Date).
 */
...
const ActivityTab = ({ institutionId }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiRequest(`/v1/admin/institutions/${institutionId}/activity?skip=${skip}&limit=${limit}`)
      .then((r) => {
        setItems(r.items ?? []);
        setTotal(r.total ?? 0);
      })
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            ...
            `) : html`
              <tr><td colSpan=${3} className=${TABLE_CELL} style=${{ color: 'var(--app-text-muted)' }}>No activity yet</td></tr>
            `}
          </tbody>
        </table>
      </div>
