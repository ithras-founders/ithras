/**
 * People tab: Current members, Alumni, Admins.
 */
          ...
          <table className="w-full">
            <thead style=${{ background: 'var(--app-surface-subtle)' }}>
              <tr>
                ...
                <th className=${TABLE_HEAD} style=${{ color: 'var(--app-text-muted)' }}>Degree</th>
                <th className=${TABLE_HEAD} style=${{ color: 'var(--app-text-muted)' }}>Majors</th>
              ...
              ${current.length ? current.map((p) => html`
                <tr key=${p.user_id} className="border-b border-[var(--app-border-soft)] hover:bg-[var(--app-surface-hover)]">
                  <td className=${TABLE_CELL}>
                    <a href=${`/p/${p.profile_slug}`} target="_blank" rel="noopener noreferrer" className="font-medium hover:underline" style=${{ color: 'var(--app-accent)' }}>
                      ${p.full_name}
                    </a>
                  </td>
                  <td className=${TABLE_CELL} style=${{ color: 'var(--app-text-secondary)' }}>${p.degree || '-'}</td>
                  <td className=${TABLE_CELL} style=${{ color: 'var(--app-text-secondary)' }}>${(p.majors || []).join(', ') || '-'}</td>
                  <td className=${TABLE_CELL} style=${{ color: 'var(--app-text-muted)' }}>${p.start_month || ''} - ${p.end_month || 'Present'}</td>
                </tr>
              `) : html`
                <tr><td colSpan=${4} className=${TABLE_CELL} style=${{ color: 'var(--app-text-muted)' }}>No current members</td></tr>
              `}
            </tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              `) : html`
                <tr><td colSpan=${4} className=${TABLE_CELL} style=${{ color: 'var(--app-text-muted)' }}>No alumni</td></tr>
              `}
            </tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              `) : html`
                <tr><td colSpan=${2} className=${TABLE_CELL} style=${{ color: 'var(--app-text-muted)' }}>No admins yet</td></tr>
              `}
            </tbody>
          </table>
        </div>
