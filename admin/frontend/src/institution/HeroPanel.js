/**
 * Hero panel: logo, name, type badge, stats, actions, profile completeness.
 */
import React from 'react';
import htm from 'htm';
import ProfileCompletenessBar from './components/ProfileCompletenessBar.js';
...
const HeroPanel = ({ institution, stats, onViewPublic, onSaveDraft, onPublish }) => {
  const degCount = institution?.degrees_v2?.length ?? 0;
  const majCount = institution?.majors_v2?.length ?? 0;
  const deptCount = institution?.departments?.length ?? 0;
  ...
  return html`
    ...
    >
      <div className="p-6 border-b border-[var(--app-border-soft)]" style=${{ background: 'var(--app-surface-subtle)' }}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-4 min-w-0">
            ${institution?.logo_url ? html`
              <img src=${institution.logo_url} alt="" className="h-16 w-16 rounded-xl object-cover flex-shrink-0" />
            ` : html`
              <div
            ...
            <div className="flex-1 min-w-0">
              ...
              ${institution?.institution_type ? html`
                <span
              ...
              ` : null}
              ${(institution?.city || institution?.country) ? html`
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            ` : null}
            ${onViewPublic && institution?.slug ? html`
              <button
        ...
        <div className="mt-4">
          <${ProfileCompletenessBar} institution=${institution} />
        </div>
        ${stats ? html`
          <div className="flex flex-wrap gap-6 mt-4 text-sm" style=${{ color: 'var(--app-text-secondary)' }}>
            ...
            <span>${degCount} degrees</span>
            <span>${majCount} majors</span>
            <span>${deptCount} departments</span>
          </div>
        ` : null}
        ${institution?.status === 'placeholder' ? html`
