/**
 * Full-page institution edit: used at /admin/institutions/:id
 */
...
const InstitutionEditPage = ({ institutionId, onBack }) => {
    ...
    <div className="max-w-2xl">
      ...
      <div className="rounded-2xl border border-[var(--app-border-soft)] bg-white shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2" style=${{ color: 'var(--app-text-primary)' }}>Allowed degrees</label>
            <${ListEditor} items=${form.degrees || []} onItemsChange=${(d) => setForm({ ...form, degrees: d })} placeholder="e.g. Bachelor of Science" addLabel="Add degree" />
          </div>
          <div>
            <label className="block text-sm font-semibold mb-2" style=${{ color: 'var(--app-text-primary)' }}>Allowed majors</label>
            <${ListEditor} items=${form.majors || []} onItemsChange=${(m) => setForm({ ...form, majors: m })} placeholder="e.g. Computer Science" addLabel="Add major" />
          </div>
          <div>
            <label className="block text-sm font-semibold mb-2" style=${{ color: 'var(--app-text-primary)' }}>Allowed minors</label>
            <${ListEditor} items=${form.minors || []} onItemsChange=${(m) => setForm({ ...form, minors: m })} placeholder="e.g. Economics" addLabel="Add minor" />
          </div>
        </div>
