...
class CreatePostRequest(BaseModel):
    text: str = ""
    image_urls: list[str] = []
    post_type: Optional[str] = None  # standard, share_resource, cv_review_request, mock_interview_request
...
def _serialize_post(
    ...
    if row.get("post_type"):
        out["post_type"] = row["post_type"]
    return out


def _get_author_info(db: Session, author_id: str) -> tuple[str, str | None, bool, bool]:
    try:
        row = db.execute(
            text("SELECT name, profile_photo_url, last_active_at, COALESCE(is_verified, false) AS is_verified FROM users WHERE id = :id"),
            {"id": author_id},
        ).fetchone()
    except Exception:
        try:
    ...
    return (r.get("name") or "", r.get("profile_photo_url"), is_recently_active, is_verified)
...
def create_post(
    ...
    post_type = (req.post_type or "standard").strip() or None
    if post_type and post_type not in ("share_resource", "cv_review_request", "mock_interview_request"):
        post_type = None
    cols = "id, author_id, text, image_urls"
    vals = ":id, :author_id, :text, :image_urls::jsonb"
    params = {"id": post_id, "author_id": user.id, "text": (req.text or "").strip(), "image_urls": json.dumps(image_urls)}
    try:
        db.execute(
            text(f"INSERT INTO feed_posts ({cols}, post_type) VALUES ({vals}, :post_type)"),
            {**params, "post_type": post_type},
        )
    except Exception:
        db.execute(
            text(f"INSERT INTO feed_posts ({cols}) VALUES ({vals})"),
            params,
        )
    ...
    r = dict(row._mapping)
    author_name, author_photo_url, author_recently_active, author_is_verified = _get_author_info(db, r["author_id"])
    return _serialize_post(r, author_name=author_name, author_photo_url=author_photo_url, author_recently_active=author_recently_active, author_is_verified=author_is_verified)
